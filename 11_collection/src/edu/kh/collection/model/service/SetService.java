...
public class SetService {
	...
	// ***Set이 중복을 확인하는 방법***
	// -> 객체가 가지고 잇는 필드값이 모두 같으면 중복으로 판단 
	...
	public void ex1() {
		...
		// 사용 조건 2 : 저자오디는 객체에 hashCode() 오버라이딩 필수
		
		// ** Hash 라는 단어가 붙은 컬렉션은 반드시
		//  저장되는 객체에 equals(), hashCode() 오버라이딩 해야함.
		...
		System.err.println(set.remove("당근마켓") );
		System.err.println(set.remove("라인") );
		
		System.out.println(set);
		
		// Set은 순서가 없어서 저장된 객체 하나를 얻어올 수 있는 방법이 없다!
		// -> 대신에 Set 전체 데이터를 하나씩 반복적으로 얻어올 순 있다!
	...
	public void ex2() {
		...
		// ex) 입력 : 111 -> "asdfg" (5글자)
		// ex) 입력 : 1233242 -> "qwzg" (5글자)
		
		// hashCode() : 필드 값이 다르면 중복되진 않는 숫자를 만드는 메서드 
	...
	// 기본 자료형 -> 포장
	Integer w1 = new Integer(iNum); // int 가 Integer 로 포장
	...
	System.out.println("double 최소값 : " + w2.MIN_EXPONENT);
	// 기울어진 글씨? == static
	...
	System.out.println("double 최소값 : " + Double.MIN_EXPONENT);
	...
	public void lotto() {
		...
		while( lotto.size() < 6) {
			...
			lotto.add(random);
			//int 값이 자동으로 Integer로 포장되어 lotto에 추가
