...
public class StudentService {
	...
	private static final String StudentServic = null;
	...
	// java.uril.ArrayList : 배열 형태 List  (가장 대표적인 List 자식 클래스)
	
	// ArrayList() 기본생성자 : 기본 크기 10짜리 리스트 생성 
	//  				-> 하지만 리스트는 크기가 늘었다 줄었다 하기때문에 큰 의미 없음.
	
	// ArrayList(용량) : 용량만큼의 리스트 생성 
	// 					-> 너무 큰값을 작성하면 메모리 많이 소모함.
	
	// private List<Student> sudentList = new ArrayList<Student>(); //검색(조회)에 효율적
	 ...
	 public void displaymenu() {
		 ...
		 do {
			 ...
			 System.out.println("5. 이름으로 검색( 포함 ) ");
			 
			 System.out.println("0. 프로그램 종료");
			 
			 System.out.println("\n메뉴 번호 선택 >>");
			 
			 // 오류문자를 잡아주는 try catch
			 ...
			 try {
				 ...
				 case 1 : System.out.println("학생 정보 추가"); break;
				 case 2 : System.out.println("학생 전체 조회"); break;
				 case 3 : System.out.println("학생 정보 수정"); break;
				 case 4 : System.out.println("학생 정보 제거"); break;
				 case 5 : System.out.println("이름으로 검색(포함)"); break;
				 ...
				 System.out.println("error : 입력형식이 유효하지" 
						 				+"않습니다. 다시 시도해주세요");
	 ...
	 public String addStudent()throws InputMismatchException {
		 ...
		 System.out.print("나이 : ");
		 int age = sc.nextInt();
		 // int 다음 line이 올시 sc.nextLine(); 한번더 입력해 입력 버퍼 개행 문자 제거
		 sc.nextLine();
	 ...
	 public void selectAll() {
		 ...
		 // - List가 비어있는지 확인하는 방법
		 //				boolean List.isEmpry() : 비어있으면 true를 반환
		 ...
		 if(studentList.isEmpty()) {
			 ...
			 return; // 현재 메소드를 종료하고 호춣한 곳으로 돌아감.
			 // 단, 반환값은 없다(void)
		 ...
		 // 향상된 for문 (for each 문)
		 // - 컬렉션, 배열의 모든 요소를 순차적을 반복접근할 수 있는 for문
	 ...
	 public String updateStudent() throws InputMismatchException {
		 ...
		 // Student List.set(int index, Studet e)
		 // -> List의 i번째 요소를 전달받은 e 로 변경
		...
		// 4) 입력받은 숫자가 studentList 범위 내 인덱스 번호인가?
			...
			int age = sc.nextInt();
			sc.nextLine();//입력 버퍼 개행 문자 제거 (int 다음 Line나올시 )
			...
			System.out.println("점수 : ");
			int score = sc.nextInt(); // InputMistmathException
	...
	public String removeStudent() throws InputMismatchException {
		 ...
		 //4) 입력받은 숫자가 studentList 범위 내 인덱스 번호인가?
			 ...
			 // String.toUpperCase(): 문자열ㅇㄹ 대문자로 변경
